...
if echo $fehFiles | grep -w $ext > /dev/null; then
    feh "$1" >/dev/null &
elif pgrep -f MPV-Q2 > /dev/null
then
    notify-send "$(echo -e "Now playing in PIP mpv\n $1")"
    printf "%s\n" "loadfile \"$1\"" > $HOME/.mpvinput2
else
...
#        mpv ---ytdl-format="bestvideo[height<=?720][vcodec!=vp9]+bestaudio/best[height<=?720]/best" -x11-name=MPV-Q --input-file=$HOME/.mpvinput --loop "$1" > /dev/null &
    ...
    if echo "$@" | grep "$vidsites">/dev/null; then
        ...
        mpv --ytdl-format="bestvideo[height<=?480][vcodec!=vp9]+bestaudio/best[height<=?480]/best" --ytdl-raw-options=netrc= --x11-name=MPV-Q2 --input-file=$HOME/.mpvinput2 "$1" > /dev/null &
    fi
fi
